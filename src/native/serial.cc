...
set_attributes (int fd, int baud_rate, MonoParity parity, int dataBits, MonoStopBits stopBits, MonoHandshake handshake)
{
	...
	/* stopbits */
	switch (stopBits) {
		case MonoStopBits::NoneStopBits:
			/* Unhandled */
			break;
		case MonoStopBits::One: /* One */
			/* do nothing, the default is one stop bit */
			newtio.c_cflag &= ~CSTOPB;
			break;
		case MonoStopBits::Two: /* Two */
			newtio.c_cflag |= CSTOPB;
			break;
		case MonoStopBits::OnePointFive: /* OnePointFive */
	...
	switch (parity)	{
		case MonoParity::NoneParity: /* None */
			newtio.c_cflag &= ~(PARENB | PARODD);
			break;
	    
		case MonoParity::Odd: /* Odd */
			newtio.c_cflag |= PARENB | PARODD;
			break;
	    
		case MonoParity::Even: /* Even */
			newtio.c_cflag &= ~(PARODD);
			newtio.c_cflag |= (PARENB);
			break;
	    
		case MonoParity::Mark: /* Mark */
			/* XXX unhandled */
			break;
		case MonoParity::Space: /* Space */
			/* XXX unhandled */
			break;
	}
	...
	switch (handshake) {
		case MonoHandshake::NoneHandshake: /* None */
			/* do nothing */
			break;
		case MonoHandshake::RequestToSend: /* RequestToSend (RTS) */
#ifdef CRTSCTS
			newtio.c_cflag |= CRTSCTS;
#endif /* def CRTSCTS */
			break;
		case MonoHandshake::RequestToSendXOnXOff: /* RequestToSendXOnXOff (RTS + XON/XOFF) */
#ifdef CRTSCTS
			newtio.c_cflag |= CRTSCTS;
#endif /* def CRTSCTS */
			/* fall through */
		case MonoHandshake::XOnXOff: /* XOnXOff */
			newtio.c_iflag |= IXOFF | IXON;
			break;
	}
...
{
	switch (signal) {
		case MonoSerialSignal::Cd:
			return TIOCM_CAR;
		case MonoSerialSignal::Cts:
			return TIOCM_CTS;
		case MonoSerialSignal::Dsr:
			return TIOCM_DSR;
		case MonoSerialSignal::Dtr:
			return TIOCM_DTR;
		case MonoSerialSignal::Rts:
			return TIOCM_RTS;
		default:
			return 0;
	}
...
{
	MonoSerialSignal retval = MonoSerialSignal::NoneSignal;

	if ((signals & TIOCM_CAR) != 0)
		retval |= MonoSerialSignal::Cd;
	if ((signals & TIOCM_CTS) != 0)
		retval |= MonoSerialSignal::Cts;
	if ((signals & TIOCM_DSR) != 0)
		retval |= MonoSerialSignal::Dsr;
	if ((signals & TIOCM_DTR) != 0)
		retval |= MonoSerialSignal::Dtr;
	if ((signals & TIOCM_RTS) != 0)
		retval |= MonoSerialSignal::Rts;
...
get_signals (int fd, int32_t *error)
{
	...
	*error = 0;
	...
	if (ioctl (fd, TIOCMGET, &signals) == -1) {
		*error = -1;
		return MonoSerialSignal::NoneSignal;
	}
	...
	while ((filename = g_dir_read_name (dir))) {
		if (filename) {
			if (!strncmp (filename, "ttyS", 4))
				list = g_slist_append (list, g_strconcat ("/dev/", filename, nullptr));
		}
	}
